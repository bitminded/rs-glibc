...
pub fn dlopen(filename: &str, flags: i32) -> Result<FileHandle, Box<dyn Error>>
{
    if filename == ""
    {
        let handle = unsafe
        {
            FileHandle{ _private: _dlopen(std::ptr::null(), flags) }
        };
        return Ok(handle);
    }
    else
    {
        ...
        let handle = unsafe
        {
            FileHandle{ _private: _dlopen(cstr_filename.as_ptr(), flags) }
        };
        return Ok(handle);
    }
}
...
pub fn dlsym(handle: &FileHandle, symbol: &str) -> Result<*mut c_void, Box<dyn Error>>
{
    ...
    {
        _dlsym(handle._private, cstr_symbol.as_ptr())
    };
    Ok(symbol_handle)
}